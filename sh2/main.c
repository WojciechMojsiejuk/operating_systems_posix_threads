...
{
	//variable containing latest error code
	int result;
	while(1)
	{
		//Check waiting queue once
		...
		if(current_queue_size(&waitingQueue) > 0)
		{
			...
			//Finished
			//Send signal to client that he can pay and leave
			result = pthread_mutex_lock(&mFinishedCutting[currentlyCutId]);
			if (result)
			{
					fprintf(stderr, "mFinishedCutting could not be locked\n");
					exit(EXIT_FAILURE);
			}
			finishedCutting[currentlyCutId] = 1;		pthread_cond_broadcast(&cFinishedCutting[currentlyCutId]);										pthread_mutex_unlock(&mFinishedCutting[currentlyCutId]);
			//Nobody is getting cut - set currentlyCutId to -1
			pthread_mutex_lock(&hairdressersChairTaken);
			currentlyCutId = -1;
			pthread_mutex_unlock(&hairdressersChairTaken);
		}
		...
		{
	    pthread_mutex_unlock(&accessWaitingQueue);
			result = pthread_mutex_lock(&mBarberWokenUp);
			if (result)
			{
					fprintf(stderr, "mBarberWokenUp could not be locked\n");
					exit(EXIT_FAILURE);
			}
			barberWokenUp = 0;
...
void* Client(void* numer) {
  ...
  id = *((int *) numer);
	...
	while(1)
	{
		...
		//There is an empty chair - join queue
		else
		{
			if(debug >= 2)
			...
			pthread_mutex_unlock(&mBarberWokenUp);
			//wait untill barber says cutting is done
			result = pthread_mutex_lock(&mFinishedCutting[id]);
			if (result)
			{
					fprintf(stderr, "mFinishedCutting could not be locked\n");
					exit(EXIT_FAILURE);
...
int main(int argc, char* argv[])
{
    ...
    //Alloc memory for clients' threads
    pthread_t* clientID;
